// Text: http://adventofcode.com/2016/day/4
// Input: http://adventofcode.com/2016/day/4/input
const input = 
...
const countLetters = (name) => {
  const freq = {};
  name.split('').filter(l => l!='-').forEach(letter => {
    freq[letter] = freq[letter] ? freq[letter] + 1 : 1;
  });
  return freq;
};

// utility function, to make this call less verbose
const getASCII = l => l.charCodeAt(0);

// regex to match <room name><sectionID>[<hash>]
...
const rooms = input.split('\n').map(line => {
  ...
  const name     = match[1];
  const sectorID = parseInt(match[2]);
  const hash     = match[3].split('');

  let letters = countLetters(name);

  const validHash = hash.every( (h, i) => {
    const freq = letters[h] || 0;
    return Object.keys(letters)
      // exclude the letters already checked and the current one
      .filter(l => hash.slice(0,i+1).indexOf(l) < 0)
      .every(l => 
        // this letter must be the most frequent
        (letters[l] < freq) || 
        // if tie, look which comes first in aplhabetical order
        (letters[l] == freq && getASCII(h) < getASCII(l) )
      );
  });
...
const sectorIDsum = rooms.filter(r => r.validHash).reduce( (acc, r) => acc + r.sectorID, 0 );

// Part 2:

// rotates letter
const rotateLetter = (l, shift) => String.fromCharCode((getASCII(l) - getASCII('a') + shift) % 26 + getASCII('a'));

// dechiper real name
rooms.map(r => {
  r.realName = r.name.split('').map(l => l == '-' ? ' ' : rotateLetter(l, r.sectorID)).join('').trim();
})

// find northpole object storage
const northPoleID = rooms.find(r => r.realName.indexOf('north') > -1).sectorID;

console.log('Part 1 solution:', sectorIDsum);
console.log('Part 2 solution:', northPoleID);
